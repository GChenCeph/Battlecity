...
class MainGame(object):
    ...
    def startgame(self):
        ...
        while not GAMEOVER:
            ...
            if len(MainGame.enemy_tank_list) == 0 and MainGame.enemy_tank_counter == 12:
                GAMEOVER = True
                self.game_over()
    ...
    def blit_enemy_tank(self):
        for etank in MainGame.enemy_tank_list:
            if etank.live:
                ...
                if MainGame.enemy_tank_counter < 12:
                    top = 0
                    speed = random.randint(1, 3)  # // 2
                    odd_numbers = [x for x in range(0, 6) if x % 2 == 1]
                    left = np.random.choice(odd_numbers)
...
class Bullet(BaseItem):
    ...
    def hit_enemy_tank(self):
        for e_tank in MainGame.enemy_tank_list:
            if pygame.sprite.collide_rect(e_tank, self) and self.alive:  # Check if the bullet is still alive
